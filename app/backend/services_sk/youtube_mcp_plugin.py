...
try:
    ...
    MCP_AVAILABLE = False
    logging.warning("MCP 라이브러리가 설치되지 않았습니다. pip install mcp를 실행하세요.")
...
class YouTubeMCPPlugin:
    ...
    def __init__(self):
        ...
        if not MCP_AVAILABLE:
            logger.warning("MCP 라이브러리가 설치되지 않았습니다. YouTube MCP 기능이 비활성화됩니다.")
            return
            
        if not YOUTUBE_API_KEY:
            logger.warning("YOUTUBE_API_KEY 환경 변수가 설정되지 않았습니다. YouTube 기능이 비활성화됩니다.")
            return
    ...
    async def _ensure_mcp_connection(self):
        ...
        if self.mcp_session is None and MCP_AVAILABLE:
            try:
                ...
                logger.info(f"YouTube MCP 서버에 연결됨. 사용 가능한 도구: {tool_names}")
    ...
    async def _search_youtube_videos(self, query: str, max_results: int = 10) -> Dict[str, Any]:
        """searchVideos MCP 도구로 YouTube 비디오 검색"""
        try:
            if not await self._ensure_mcp_connection():
                return {"status": "error", "message": "MCP 서버에 연결할 수 없습니다.", "videos": []}
            ...
            if getattr(search_result, 'is_err', False):
                ...
                return {"status": "error", "message": f"검색 실패: {search_result.content}", "videos": []}
            ...
            if hasattr(search_result, 'content') and search_result.content:
                try:
                    ...
                    return {"status": "error", "message": "응답 데이터 파싱 실패", "videos": []}
                except Exception as e:
                    logger.error(f"결과 파싱 중 오류: {e}")
                    logger.debug(f"원본 응답: {search_result.content}")
                    return {"status": "error", "message": f"결과 파싱 실패: {str(e)}", "videos": []}
            
            return {"status": "success", "videos": videos_data, "search_query": query, "total_results": len(videos_data)}
            ...
            return {"status": "error", "message": f"검색 중 오류가 발생했습니다: {str(e)}", "videos": []}

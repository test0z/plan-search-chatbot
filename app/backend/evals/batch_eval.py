...
from azure.ai.evaluation import RelevanceEvaluator, SimilarityEvaluator, RetrievalEvaluator
...
class BatchEvaluator:
    ...
    def batch_evaluate(
        ...
        evaluation_name = f"evaluation_{datetime.now(tz=timezone).strftime('%Y-%m-%d_%H-%M-%S')}"
...
class BatchResponseGenerator:
        ...
        try:
            ...
            try:
                ...
                async def collect_response():
                    ...
                    if self.search_engine == SearchEngine.BING_GROUNDING:
                    ...
                    elif self.search_engine != SearchEngine.BING_GROUNDING and self.plan_execute:
                        async for chunk in self.plan_executor.generate_response(
                            ...
                            chunks.append(chunk)
                            # Count search queries from streaming messages
                            if "Search plan established" in chunk and "search queries" in chunk:
                                try:
    ...
    async def generate_batch(
        ...
        """
        response_results = []
        logger.info(f"Starting batch response generation of {len(queries)} queries with max_concurrent={max_concurrent}")
        logger.info(f"########### Current Response Generation Configuration: ############")
        logger.info(f"max_concurrent: {max_concurrent}, max_tokens: {max_tokens}, temperature: {temperature}")
        
        logger.info(f"query-rewrite: {self.query_rewrite}, plan-execute: {self.plan_execute}, search-engine: {self.search_engine}")
...
def load_queries_from_csv(file_path: str, limit: Optional[int] = None) -> List[Dict[str, str]]:
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Batch evaluation for Microsoft product inquiry chatbot")
    parser.add_argument("--input", "-i", required=True, help="Input CSV file containing queries")
    parser.add_argument("--gen_output", "-g", default=f"evals/results/generated_results_{current_date_time}.jsonl", help="Output JSON line file for generated results")
    parser.add_argument("--eval_output", "-e", default=f"evals/results/evaluation_results_{current_date_time}.json", help="Output JSON line file for evaluation results")
    parser.add_argument("--max_concurrent", "-c", type=int, default=1, help="Maximum number of concurrent requests")
    parser.add_argument("--max_tokens", "-t", type=int, help="Maximum tokens for response generation")
    parser.add_argument("--temperature", "-T", type=float, help="Temperature for response generation")
    parser.add_argument("--locale", "-lo", type=str, default="en-US", help="Locale for response generation")
    parser.add_argument("--limit", "-l", type=int, default=3, help="Limit on the number of queries to evaluate")
    ...
    parser.add_argument("--query_rewrite", "-q", type=str_to_bool, nargs='?', const=True, default=True, help="enable query rewriting (true/false)")
    parser.add_argument("--plan_execute", "-p", type=str_to_bool, nargs='?', const=True, default=True, help="enable plan execution (true/false)")
    parser.add_argument("--search_engine", "-s", type=str, default="grounding_bing", help="Search engine to use")
    parser.add_argument("--interval", "-it", type=float, default=1.0, help="Query execution interval (seconds)")
    ...
    try:
        ...
        batch_evaluator.batch_evaluate(
            eval_input_path=gen_output_path,

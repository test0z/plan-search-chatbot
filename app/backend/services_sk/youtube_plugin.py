...
class YouTubePlugin:
    ...
    async def _search_youtube_videos(self, query: str, max_results: int = 10) -> Dict[str, Any]:
        ...
        if not YOUTUBE_API_KEY:
            return {"status": "error", "message": "YouTube API 키가 설정되지 않았습니다.", "videos": []}
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return {"status": "error", "message": f"검색 실패: {response.status_code}", "videos": []}
    ...
    async def _get_video_details(self, video_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                return {"error": f"YouTube API 오류: {response.status_code} - {response.text}"}
...
🕐 검색 시간: {{ datetime.now().strftime('%Y-%m-%d %H:%M:%S') }}
    ...
    async def get_youtube_video_details(
        self,
        video_id: Annotated[str, "YouTube 비디오 ID (예: 'dQw4w9WgXcQ')"]
    ) -> str:

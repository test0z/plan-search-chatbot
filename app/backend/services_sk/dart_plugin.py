...
class DartPlugin:
    ...
    async def get_financial_stats_with_fallback(
        self, corp_code: str, max_attempts: int = 5
    ) -> dict:
        """
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if result.get("status") == "000" and result.get("list"):
                    print(
                        f"### Successfully fetched financial data for {year}, {reprt_code}"
                    )
                    return result
                else:
                    print(
                        f"❌ No data for {year}, {reprt_code}: {result.get('message', 'Unknown error')}"
                    )
    ...
    async def fetch_dart_data(
        self, corp_code: Annotated[str, "DART 기업 코드"] = None
    ) -> str:
        try:
            timezone = pytz.timezone(TIME_ZONE)
            today = datetime.now(tz=timezone)
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            if result.get("status") == "000" and result.get("list"):
                                ...
                                year, reprt_code = await self._fallback(
                                    year, reprt_code
                                )
            ...
            summary = context_template.render(
                profile=profile, filings=filings, financials=financials
            )
            return summary

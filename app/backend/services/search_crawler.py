...
class SearchCrawler(ABC):
    ...
    @abstractmethod
    def search(
        self, query: str, locale: Optional[str] = "ko-KR"
    ) -> List[Dict[str, str]]:
        """
    ...
    async def extract_contexts_async(
        self, url_snippet_tuples: List[Tuple[str, str]]
    ) -> List[str]:
        """
        ...
        async def fetch_and_cache(url: str, snippet: str) -> str:
            if self.redis_client:
                try:
                    ...
                    if cached_text:
                        logger.info(
                            f"Retrieved content from Redis cache for URL: {url}"
                        )
                        return cached_text
                except Exception as e:
                ...
                async with httpx.AsyncClient(
                    ...
                    try:
                        ...
                        if (
                            ...
                            and "location" in e.response.headers
                        ):
                    ...
                    paragraphs = [
                        p.strip()
                        for p in selector.css("p::text, p *::text").getall()
                        if p.strip()
                    ]
                    ...
                    if self.redis_client and len(text) > 200:
                        try:
                            self.redis_client.set(
                                url, snippet_text, ex=self.cache_expiration
                            )
        ...
        tasks = [
            asyncio.create_task(fetch_and_cache(url, snippet))
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                logger.error(
                    f"Error processing URL {url_snippet_tuples[i][0]}: {str(result)}"
                )
                processed_results.append(
                    f"{url_snippet_tuples[i][1]} [Processing Error]"
                )
            else:

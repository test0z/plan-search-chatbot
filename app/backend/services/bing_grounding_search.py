...
class BingGroundingSearch:
    ...
    def __init__(self, redis_config: Dict[str, Any] = None):
        ...
        if self.search_gen_agent_id_env:
            self.search_gen_agent = self.agents_client.get_agent(
                self.search_gen_agent_id_env
            )
            logger.info(
                f"Using existing search-gen-agent, ID: {self.search_gen_agent.id}"
            )
            if self.search_gen_agent is None:
                logger.error(
                    "search_gen_agent is None, please check the environment variable SEARCH_GEN_AGENT_ID"
                )
    ...
    async def search_and_generate_by_bing_grounding_ai_agent(
        self,
        ...
        logger.info(
            f"final user_instruction on Azure AI Agent: {search_gen_instruction}"
        )
        ...
        async def get_agent_message_with_retry(
            thread_id, role, max_retries=5, delay=1.0
        ):
            """
            ...
            for attempt in range(max_retries):
                ...
                logger.info(
                    f"Agent message not found (attempt {attempt+1}/{max_retries}), retrying after {delay}s..."
                )
        ...
        if stream:
            with self.agents_client.runs.stream(
                ...
                for event_type, event_data, _ in stream:
                    if isinstance(event_data, MessageDeltaChunk):
                        ...
                        logger.info(
                            f"ThreadMessage created. ID: {event_data.id}, Status: {event_data.status}"
                        )
                        ...
                        logger.info(
                            f"RunStep type: {event_data.type}, Status: {event_data.status}"
                        )
                        ...
                        logger.info(
                            f"Unhandled Event Type: {event_type}, Data: {event_data}"
                        )
            ...
            if not response_message or not getattr(response_message, "content", None):
                logger.error(
                    "Agent response message not found after streaming (NoneType)."
                )
            ...
            run_steps = self.agents_client.run_steps.list(
                ...
                if tool_calls:
                    ...
                    for call in tool_calls:
                        ...
                        if bing_grounding_details:
                            logger.info(
                                f"  Bing Grounding ID: {bing_grounding_details.get('requesturl')}"
                            )
...
class BingGroundingCrawler(SearchCrawler):
    ...
    def __init__(self, redis_config: Dict[str, Any] = None):
        ...
        if self.search_agent_id_env:
            ...
            if self.search_agent is None:
                logger.error(
                    "search_agent is None, please check the environment variable SEARCH_AGENT_ID"
                )
                ...
                logger.info(
                    f"Updating existing search-agent, ID: {self.search_agent.id}"
                )
                self.agents_client.update_agent(
    ...
    def search(
        self, query: str, locale: Optional[str] = "en-US"
    ) -> List[Dict[str, str]]:
        """
        ...
        search_instruction = USER_PROMPT.format(
            current_date=current_date, query=query, max_results=self.max_results
        )
        ...
        run = self.agents_client.runs.create_and_process(
            thread_id=thread.id, agent_id=self.search_agent.id
        )

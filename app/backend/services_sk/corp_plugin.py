...
class CORPPlugin:
    ...
    def __init__(self, db_path: str = "corp_codes.db", xml_path: str = None):
        ...
        logger.info(f"CORPPlugin initialized with:")
    ...
    def _load_xml_to_database(self):
        ...
        try:
            ...
            logger.info(f"Found {len(companies)} companies in XML. Inserting to database...")
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                for i in range(0, len(companies), batch_size):
                    ...
                    cursor.executemany('''
                        ...
                        VALUES (?, ?, ?, ?)
                    ''', [
                        (comp['corp_code'], comp['corp_name'], comp['stock_code'], comp['modify_date'])
                        for comp in batch
                    ])
    ...
    def _parse_corp_code_xml(self, file_path: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for company in list_elements:
                ...
                stock_code = stock_code_elem.text.strip() if stock_code_elem.text and stock_code_elem.text.strip() else ""
                modify_date = modify_date_elem.text.strip() if modify_date_elem.text else ""
    ...
    async def _find_company_by_name_async(self, name_keyword: str, exact_match: bool = False, limit: int = 10) -> List[Dict[str, str]]:
        """
        ...
        def _search_db():
            try:
                ...
                logger.error(f"Failed to search companies by name '{name_keyword}': {e}")
                return []
    ...
    async def _get_company_by_corp_code_async(self, corp_code: str) -> Optional[Dict[str, str]]:
        """
    ...
    async def find_corp_code_by_name(
        ...
        exact_match: Annotated[bool, "Whether to search for exact match or partial match"] = False
    ) -> str:
        """
        ...
        """
        try:
            logger.info(f"Searching for company: '{corp_name}', exact_match: {exact_match}")
            
            # Search for companies
            companies = await self._find_company_by_name_async(corp_name, exact_match=exact_match, limit=5)
            ...
            # Format results
            result_lines = []
            result_lines.append(f"Found {len(companies)} company(ies) matching '{corp_name}':")
            
            for i, company in enumerate(companies, 1):
                stock_info = f" (Stock: {company['stock_code']})" if company['stock_code'] else ""
                result_lines.append(
            ...
            result_lines.append("")
            result_lines.append(f"Best match corp_code for DART API: {best_match['corp_code']}")
    ...
    async def get_company_info(
        self, 
        corp_code: Annotated[str, "DART corporation code"]
    ) -> str:
        """
        ...
        try:
            ...
            stock_info = f"\nStock Code: {company['stock_code']}" if company['stock_code'] else "\nStock Code: Not Listed"
...
Last Modified: {company['modify_date']}"""
            ...
            error_msg = f"Error getting company info for corp_code '{corp_code}': {str(e)}"
            logger.error(error_msg)
            return error_msg
...
def find_company_by_name(company_list: List[Dict[str, str]], name_keyword: str) -> List[Dict[str, str]]:
    """

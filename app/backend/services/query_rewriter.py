...
QUERY_REWRITE_PROMPT = load_prompt(os.path.join(current_dir, "..", "prompts", "query_rewrite_prompt.yaml"), encoding="utf-8")
REWRITE_PLAN_PROMPT = load_prompt(os.path.join(current_dir, "..", "prompts", "rewrite_plan_prompt_ko.yaml"), encoding="utf-8")
...
class QueryRewriter:
    ...
    def __init__(self, client: Optional[AzureOpenAI] = None, settings: Optional[Settings] = None):
        """
        ...
        if settings:
            ...
            logger.warning("Settings not provided to QueryRewriter, deployment name may be missing")
            self.query_deployment_name = None
            
    
    async def rewrite_query(self, original_query: str, 
                          temperature: float = 0.9, locale: Optional[str] = "ko-KR") -> Dict[str, str]:
        """
        ...
        try:
            ...
            formatted_prompt = QUERY_REWRITE_PROMPT.format(user_query=original_query, locale=locale)
            logger.info(f"==============Rewriting query: {original_query} with locale: {locale}")
            # Make the API call
    ...
    async def rewrite_plan_query(self, original_query: str, 
                          temperature: float = 0.9, locale: Optional[str] = "ko-KR") -> Dict[str, str | list]:
        """
        ...
        try:
            ...
            formatted_prompt = REWRITE_PLAN_PROMPT.format(user_query=original_query, planner_max_plans=self.planner_max_plans, date=current_date, locale=locale)
            print(formatted_prompt)
            logger.info(f"==============Rewrite and Plan query: {original_query} with locale: {locale}")
            # Make the API call
            response = await self.client.chat.completions.create(
                model=self.query_deployment_name,
                messages=[

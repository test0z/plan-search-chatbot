...
class ChatRequest(BaseModel):
    ...
    temperature: Optional[float] = Field(
        None, description="Temperature for response generation (0.0 to 1.0)"
    )
    stream: bool = Field(False, description="Enable streaming response")
    query_rewrite: bool = Field(
        True, description="Enable query rewriting for better search results"
    )
    plan_execute: bool = Field(
        False, description="Enable plan and execute mode for complex queries"
    )
    search_engine: str = Field(
        SearchEngine.GOOGLE_SEARCH_CRAWLING,
        description="Search engine to use for retrieving information",
    )
    locale: str = Field("ko-KR", description="Locale for the response")
...
class PlanSearchRequest(BaseModel):
    ...
    planning: bool = Field(
        True, description="Enable search planning for better search results"
    )
    ...
    include_ytb_search: bool = Field(True, description="Include YouTube search results")
    include_mcp_server: bool = Field(True, description="Include MCP server integration")
    verbose: bool = Field(False, description="Verbose mode for detailed output")

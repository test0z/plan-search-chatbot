...
class SearchPlugin:
    ...
    def __init__(self, bing_api_key: str = None, bing_endpoint: str = None, fallback_crawler=None, bing_custom_config_id: str = None):
        """
        ...
        self.bing_custom_config_id = bing_custom_config_id or os.getenv("BING_CUSTOM_CONFIG_ID")
        ...
        logger.info(f"SearchPlugin initialized with:")
        logger.info(f"  - Bing API Key: {'SET' if self.bing_api_key else 'NOT SET'}")
        logger.info(f"  - Bing Endpoint: {self.bing_endpoint}")
        logger.info(f"  - Bing Custom Config: {'SET' if self.bing_custom_config_id else 'NOT SET'}")
        logger.info(f"  - Fallback Crawler: {'SET' if self.fallback_crawler else 'NOT SET'}")
        
    async def cleanup(self):
        """Cleanup resources - no persistent sessions to clean"""
        pass

    @kernel_function(name="search_single_query", description="Search the web for the given query")
    async def search_single_query(self, query: str, locale: str = "ko-KR", max_results: int = 5, max_context_length: int = 3000) -> str:
        """
        ...
        try:
            ...
            enriched_results = await self._enrich_results_with_content(results, max_results, max_context_length)
    ...
    async def _search_bing_api(self, query: str, locale: str = "ko-KR", max_results: int = 5) -> List[Dict[str, Any]]:
        """
        ...
        """
        logger.info(f"Starting Bing search for query: '{query}', locale: {locale}, max_results: {max_results}")
    ...
    async def _enrich_results_with_content(self, results: List[Dict[str, Any]], max_results: int, max_context_length: int) -> List[Dict[str, Any]]:
        """
        Enrich search results with content extracted from URLs (parallel processing).
        """
        async def enrich(result, rank):
            try:
                content = await self._extract_content_from_url(result.get('url', ''), max_context_length)
                return {
    ...
    async def _extract_content_from_url(self, url: str, max_context_length: int) -> Optional[str]:
        """

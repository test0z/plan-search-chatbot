...
async def test_generate_response_with_query_rewrite(plan_executor):
    ...
    # Mock search results
    plan_executor.search_crawler.search = MagicMock(
        return_value=[
            {"link": "https://Microsoft.com/surface", "snippet": "Surface info"}
        ]
    )
    plan_executor.search_crawler.extract_contexts_async = AsyncMock(
        return_value=[
    ...
    answer_completion.choices = [
        MagicMock(
            message=MagicMock(
                content="✨ The Microsoft Surface is a flagship phone with...\n\n[More information](https://Microsoft.com/galaxy-s25)"
            )
        )
    ]
    ...
    response = [
        msg
        async for msg in plan_executor.generate_response(
            messages,
            query_rewrite=True,
...
async def test_generate_response_no_contexts(plan_executor):
    ...
    final_completion.choices = [
        MagicMock(
            message=MagicMock(content="검색결과가 부족하여 답변을 할 수 없습니다.")
        )
    ]
    ...
    async def test_generate_response_error_on_planner(plan_executor):
        ...
        plan_executor.client.chat.completions.create.side_effect = Exception(
            "Planner API Failure"
        )
    ...
    async def test_generate_response_error_on_answer_generation(plan_executor):
        ...
        plan_executor.search_crawler.extract_contexts_async = AsyncMock(
            return_value=["The Sun is a star at the center of the Solar System."]
        )

        response = [
            msg
            async for msg in plan_executor.generate_response(
                messages, query_rewrite=False, stream=False
            )
        ]
    ...
    async def test_generate_response_streaming_error(plan_executor):
        ...
        plan_executor.search_crawler.extract_contexts_async = AsyncMock(
            return_value=["Mars is the fourth planet from the Sun."]
        )

        response = [
            msg
            async for msg in plan_executor.generate_response(
                messages, query_rewrite=False, stream=True
            )
        ]
        assert any("Error:" in r for r in response)

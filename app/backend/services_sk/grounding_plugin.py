...
class GroundingPlugin:
    ...
    async def grounding_search_multi_query(
        ...
        try:
                ...
                try:
                    ...
                    if isinstance(parsed_queries, list):
                        ...
                        if "search_queries" in parsed_queries:
                            ...
                            queries_list = list(parsed_queries.values()) if parsed_queries else [search_queries]
                    else:
            ...
            if len(queries_list) == 1:
                ...
                formatted_queries = "\n".join([f"{i+1}. {query}" for i, query in enumerate(queries_list)])
    ...
    async def _execute_grounding_search(
        ...
        try:
            ...
            logger.info(f"Final user instruction on Azure AI Agent: {search_gen_instruction}")
            ...
            async def get_agent_message_with_enhanced_retry(thread_id, role, max_retries=20, initial_delay=0.3):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        actual_delay = delay * (1 + 0.1 * (attempt % 3))  # Add some jitter
                        logger.info(f"Agent message not found (attempt {attempt+1}/{max_retries}), retrying after {actual_delay:.2f}s...")
                        ...
                        logger.warning(f"Error checking for agent message (attempt {attempt+1}): {e}")
            ...
            while True:
                ...
                if run_status.status == RunStatus.COMPLETED:
                    ...
                    logger.error(error_msg)
                    return f"Error: {error_msg}"
                elif run_status.status in [RunStatus.QUEUED, RunStatus.IN_PROGRESS, RunStatus.REQUIRES_ACTION]:
                    # Check timeout
                    ...
                    if elapsed > max_run_time:
                        ...
                        try:
                            self.agents_client.runs.cancel(thread_id=thread.id, run_id=run.id)
                        except:
                            pass
            ...
            if response_message is None or not getattr(response_message, "content", None):
                ...
                return "Error: No response received from search service. Please try again."
            
            logger.info(f"Agent response message received: {len(str(response_message.content))} characters")

...
async def test_generate_response_no_query_rewrite(orchestrator):
    ...
    result = [x async for x in gen if x is not None and not x.startswith("data:") and not x.startswith("Response generated")]
    assert len(result) > 0
    assert result[0] == "Hello!"

@pytest.mark.asyncio
async def test_generate_response_with_query_rewrite_and_search(orchestrator):
    orchestrator.query_rewriter.rewrite_query = AsyncMock(return_value={
    ...
    orchestrator.search_crawler.extract_contexts_async = AsyncMock(return_value=["context1", "context2"])
...
async def test_generate_response_streaming(orchestrator):
    ...
    result = [x async for x in gen if x is not None and not x.startswith("data:") and not x.startswith("Response generated") and x.strip()]
    assert len(result) == 3
    assert "".join(result) == "ABC"

@pytest.mark.asyncio
async def test_generate_response_exception(orchestrator):
    orchestrator.client.chat.completions.create = AsyncMock(side_effect=Exception("fail"))
    orchestrator.search_crawler.search = MagicMock(return_value=[])
    orchestrator.search_crawler.extract_contexts_async = AsyncMock(return_value=[])
